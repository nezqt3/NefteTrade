import { createUser, getUser } from './auth.repository'
...
export async function registerService(data: AuthRequest): Promise<AuthResponse> {
    const hashPasswordOfUser = await hashPassword(data.password)

    const result = await createUser(data.email, hashPasswordOfUser, data.login, data.numberPhone, data.data, data.role)

    if (!result) {
        throw new Error("Failed to create user")
    }

    const accessToken = generateAccessToken({ userId: result.id, role: data.role })
    const refreshTokenId = generateSecretOfRefreshToken();
